...
impl TitanhApiBuilder {
	pub fn rpc(url: &str) -> Self {
		TitanhApiBuilder { rpc_url: url.to_string(), seed_phrase: None }
	}

	pub fn seed(self, seed: &str) -> Self {
		Self { seed_phrase: Some(seed.to_string()), ..self }
	}
	...
	pub async fn build(self) -> TitanhApi {
		...
		let rpc_client = RpcClient::from_url(self.rpc_url).await.expect("Invalid rpc url");
