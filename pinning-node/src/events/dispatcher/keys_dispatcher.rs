...
impl KeysDispatcher {
	pub fn new(
		...
		KeysDispatcher { client, ring, keytable }
	}
}
...
impl MutableDispatcher<KeyedPinningEvent, Option<PinningEvent>> for KeysDispatcher {
	fn dispatch(&mut self, event: KeyedPinningEvent) -> Result<Option<PinningEvent>> {
		...
		let maybe_partition = self.ring.key_node_partition(event.key, self.client.node_id())?;
	...
	for KeysDispatcher
{
	async fn async_dispatch(
		...
		let dist = self.ring.distance_between(&self.client.node_id(), &left_node)?;
...
impl KeysDispatcher {
	pub async fn replay_blocks(&self, replay: ReplayBlocks) -> Result<Batch<KeyedPinningEvent>> {
		...
		for block_num in from_block..=to_block {
			...
			let batch = self.client.events_at(BlockInfo::new(block_num, block_hash)).await?;
	...
	pub fn update_table_from_event(
		...
		match &keyed_event.pin {
			PinningEvent::Pin { cid } => {
				self.keytable.insert(row_idx, keyed_event.key, cid.clone())?;
			},
			PinningEvent::RemovePin { .. } => {
				self.keytable.remove(row_idx, &keyed_event.key)?;
			},
			PinningEvent::UpdatePin { new_cid, .. } => {
				self.keytable.insert(row_idx, keyed_event.key, new_cid.clone())?;
			},
		}
