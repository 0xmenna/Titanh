...
impl NodeProducer {
    ...
    pub async fn produce_events(&mut self) -> Result<JoinHandle<Result<()>>> {
        ...
        let new_finalized_block = events_pool.read_handle().receive_block_number().await?;
        log::info!(
            "Recover events from block number {}, to {}",
            self.startup_block + 1,
            new_finalized_block - 1,
        );
        ...
        let recover_batch = self
            ...
            .await?;
        log::info!("Recovered batch size: {}", recover_batch.size());
