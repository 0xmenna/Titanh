...
impl SubstrateClientBuilder<ApiNotInitialized> {
	...
	pub fn new() -> SubstrateClientBuilder<RpcEndpoint> {
		SubstrateClientBuilder(RpcEndpoint(Url::parse("ws://127.0.0.1:9944").unwrap()))
	}
}

impl SubstrateClientBuilder<ApiNotInitialized> {
	/// Creates a client with a custom rpc configuration
	pub fn from_url(url: &str) -> Result<SubstrateClientBuilder<RpcEndpoint>> {
		let url = Url::parse(url)?;

		Ok(SubstrateClientBuilder(RpcEndpoint(url)))
	}
}
...
impl SubstrateClientBuilder<RpcEndpoint> {
	...
	pub fn keyring_material(
		...
		phrase: &str,
		password: Option<&str>,
	) -> Result<SubstrateClientBuilder<ApiReady>> {
		// Derive the key pair from the seed phrase (mnemonic)
		let key_pair = ValidatorKeyPair::from_string(phrase, password)?;

		// Create a signer using the key pair
		let signer = PairSigner::new(key_pair);

		Ok(SubstrateClientBuilder(ApiReady { rpc_url: self.0, signer }))
	}
}
...
impl SubstrateClientBuilder<ApiReady> {
	pub async fn build(self) -> Result<SubstrateClient> {
		...
		let rpc_client = RpcClient::from_url(self.0.rpc_url.as_str()).await?;
		...
		Ok(SubstrateClient::new(api, rpc, self.0.signer))
	}
}
