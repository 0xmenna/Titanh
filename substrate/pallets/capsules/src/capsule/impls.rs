...
impl<T: Config> Pallet<T> {
    pub fn upload_capsule_from(
        ...
        let capsule_id = Self::compute_id(
            app.clone(),
            capsule.encoded_metadata.clone(),
            IdComputation::Capsule,
        );
    ...
    pub fn follow_capsule_from(who: T::AccountId, capsule_id: CapsuleIdFor<T>) -> DispatchResult {
        ...
        // Emit event
        Self::deposit_event(Event::<T>::CapsuleFollowed {
            capsule_id,
            follower: who,
        });
    ...
    pub fn update_capsule_content_from(
        ...
        Self::deposit_event(Event::<T>::CapsuleContentChanged {
            capsule_id,
            old_cid,
            cid,
            size,
    ...
    pub fn extend_ending_retention_block_from(
        ...
        Self::ensure_capsule_liveness(&capsule)?;
            ...
            at_block > capsule.ending_retention_block,
            Error::<T>::BadBlockNumber
        );
    ...
    pub fn add_priviledged_follower_from(
        ...
        let (follower_type, event) = if follower == who {
            (
                Follower::Privileged,
                Event::<T>::NewPrivilegedFollower { capsule_id, who },
            )
        } else {
            (
    ...
    fn upload_capsule_data(
        ...
        metadata: CapsuleUploadData<BlockNumberFor<T>>,
            ...
            !Self::capsule_exists(&capsule_id),
            Error::<T>::CapsuleIdAlreadyExists
        );
    ...
    pub fn start_destroy_capsule_from(
        ...
        Capsules::<T>::insert(&capsule_id, capsule.clone());

        Self::deposit_event(Event::<T>::CapsuleStartedDestroying {
            capsule_id,
            cid: capsule.cid.to_vec(),
        });
